...
hierarchical_levels = ['Level1', 'Level2', 'Level3', 'Level4', 'Level5']
...
df['Cluster'] = [clustering.labels_[nodes.index(name)] if name in nodes else -1 for name in df['Level7']]
print(df.head())
...
def prepare_training_data(df, embeddings):
    ...
    df['EncodedCategory'] = label_encoder.fit_transform(df['Level7'])  # Encode the Level7 column
    df_filtered = df[df['Level7'].isin(embeddings.keys())]

    X = np.array([embeddings[name] for name in df_filtered['Level7']])
...
if X.size == 0:
    ...
    curvature = determine_curvature(len(df['Level7'].unique()))
    ...
    def compute_hierarchical_metrics(df, model, label_encoder, embeddings):
        ...
        for _, row in df.iterrows():
            if row['Level7'] not in embeddings:
                continue

            true_category = row['Level7']
            predicted_category = predict_hierarchy(row['Level7'], model, label_encoder, embeddings)
    ...
    def get_hierarchy(name):
        ...
        for level in ['Level1', 'Level2', 'Level3', 'Level4', 'Level5', 'Level6', 'Level7']:
            if df.loc[df['Level7'] == current_name, level].empty:
                break
            parent_class = df.loc[df['Level7'] == current_name, level].values[0]
            hierarchy.append(parent_class)
            if level == 'Level7':
                break
            if pd.isnull(parent_class):
                break
            current_name = df.loc[df[level] == parent_class, 'Level7'].values[0]
...
def get_parent_classes(scientific_name, label_encoders, model, data):
    ...
    if scientific_name not in data['Level7'].values:
        raise ValueError(f"Name '{scientific_name}' not found in the data.")

    row = data[data['Level7'] == scientific_name].iloc[0]
...
def compute_hierarchical_metrics(df, model, label_encoders, embeddings):
    hierarchical_levels = ['Level1', 'Level2', 'Level3', 'Level4', 'Level5', 'Level6', 'Level7']
    lca_metrics_list = []
    path_losses = []
    level_losses = []
    ...
    for _, row in df.iterrows():
        ...
        try:
            predicted_labels = get_parent_classes(row['Level7'], label_encoders, model, df)
...
def calculate_wtie(data, alpha, beta):
    ...
    for _, row in data.iterrows():
        ...
        try:
            predicted_labels = get_parent_classes(row['Level7'], label_encoders, model, data)
        except ValueError as e:
            print(e)
            continue
...
label_encoders = create_label_encoders(df, ['Level1', 'Level2', 'Level3', 'Level4', 'Level5', 'Level6', 'Level7'])

# Perform a sample call to the get_parent_classes function to ensure it works
try:
    parent_classes = get_parent_classes('Customer Service', label_encoders, model, df)
    print(f"Parent Classes for 'Customer Service': {parent_classes}")
except ValueError as e:
    print(e)
