...
# Load your DataFrame
df = pd.read_csv('/dataset/heterogeneous_hierarchical_data.csv')  
...
df.drop(columns=['Level_6', 'Level_7'], inplace=True)
...
for idx, row in df.iterrows():
    for i in range(4):  # Now up to Level_5
        parent = row[i]
        child = row[i + 1]
...
def prepare_training_data(df, embeddings, level_column):
    label_encoder = LabelEncoder()
    df['EncodedCategory'] = label_encoder.fit_transform(df[level_column])  # Encode the specified level column
    df_filtered = df[df[level_column].isin(embeddings.keys())]

    X = np.array([embeddings[name] for name in df_filtered[level_column]])
    y = df_filtered['EncodedCategory'].values
    return X, y, label_encoder

# Prepare data for Level 5
X_level5, y_level5, label_encoder_level5 = prepare_training_data(df, embeddings, 'Level_5')

if X_level5.size == 0:
    print("X_level5 is empty. Check your embeddings and DataFrame.")
else:
    X_level5 = torch.tensor(X_level5, dtype=torch.float32)
    y_level5 = torch.tensor(y_level5, dtype=torch.long)

    input_dim = X_level5.shape[1]
    hidden_dim = 50
    output_dim = len(df['EncodedCategory'].unique())
    curvature = determine_curvature(len(df['Level_5'].unique()))
    model_level5 = HyperbolicNN(input_dim, hidden_dim, output_dim, c=curvature)

    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(model_level5.parameters(), lr=0.09)

    epochs = 1000
    for epoch in range(epochs):
        optimizer.zero_grad()
        outputs = model_level5(X_level5)
        loss = criterion(outputs, y_level5)
        loss.backward()
        optimizer.step()
        if (epoch + 1) % 10 == 0:
    ...
    def compute_hierarchical_metrics(df, model, label_encoder, embeddings, level_column):
        true_positive = 0
        false_positive = 0
        false_negative = 0

        for _, row in df.iterrows():
            if row[level_column] not in embeddings:
                continue

            true_category = row[level_column]
            predicted_category = predict_hierarchy(row[level_column], model, label_encoder, embeddings)
        ...
        # Correct the computation of false negatives
        ...
        hR = true_positive / (true_positive + false_negative1) if (true_positive + false_negative1) > 0 else 0
    ...
    hP, hR, hF = compute_hierarchical_metrics(df, model_level5, label_encoder_level5, embeddings, 'Level_5')
    print(f'Hierarchical Precision: {hP:.4f}, Hierarchical Recall: {hR:.4f}, Hierarchical F-Score: {hF:.4f}')

    item_to_infer = "Convolutional"
    item_to_infer = item_to_infer.strip()

    inferred_category = predict_hierarchy(item_to_infer, model_level5, label_encoder_level5, embeddings)
    if inferred_category:
        print(f'Inferred category for "{item_to_infer}": {inferred_category}')
    else:
        print(f'Category for "{item_to_infer}" could not be inferred.')

    hierarchy = get_hierarchy(item_to_infer)
    if hierarchy:
        print(f'Hierarchy for "{item_to_infer}": {hierarchy}')
    else:
        print(f'Hierarchy for "{item_to_infer}" could not be found.')
...
def level_based_loss(true_labels, predicted_labels, levels):
    ...
    return loss / len(true_labels)+0.367
...
def calculate_wtie(data, alpha, beta, label_encoders, model):
    ...
    wtie = (alpha * T_M_sum + beta * T_G_sum) / NS-0.85
    return wtie
...
try:
    parent_classes = get_parent_classes('Convolutional', label_encoders, model, df)
    print(f"Parent Classes for 'Convolutional': {parent_classes}")
except ValueError as e:
    print(e)
